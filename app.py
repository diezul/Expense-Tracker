...
def register():
    ...
    if request.method == 'POST':
        ...
        with get_db_connection() as conn:
            ...
            try:
                conn.execute('INSERT INTO users (username, password, currency, is_admin) VALUES (?, ?, ?, ?)',
...
def index():
    ...
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT DISTINCT service_name FROM fixed_services WHERE user_id = ?", (user_id,))
        fixed_services = [row['service_name'] for row in cursor.fetchall()]
        cursor.execute("SELECT DISTINCT service_name FROM variable_services WHERE user_id = ?", (user_id,))
        variable_services = [row['service_name'] for row in cursor.fetchall()]
        cursor.execute("SELECT * FROM expenses WHERE user_id = ? AND category = 'Fixe' ORDER BY month", (user_id,))
        fixed_expenses = cursor.fetchall()
        cursor.execute("SELECT * FROM expenses WHERE user_id = ? AND category = 'Variabile' ORDER BY month", (user_id,))
        variable_expenses = cursor.fetchall()
        cursor.execute("SELECT DISTINCT month, service_name, index_value FROM expenses WHERE user_id = ? AND category = 'Variabile' AND amount IS NULL ORDER BY month", (user_id,))
        variable_months = cursor.fetchall()
    return render_template('index.html', fixed_expenses=fixed_expenses, variable_expenses=variable_expenses, fixed_services=fixed_services, variable_services=variable_services, variable_months=variable_months)
...
@app.route('/add_service_direct/<service_type>/<service_name>', methods=['POST'])
def add_service_direct(service_type, service_name):
    ...
    with get_db_connection() as conn:
        ...
        try:
            ...
            ''', (service_name, user_id))
            conn.commit()
            return jsonify({'message': f'The service {service_name} has been successfully added to your list'})
        except sqlite3.IntegrityError:
            return jsonify({'message': f'The service {service_name} already exists in your list'})
...
def export_expenses():
    ...
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT service_name, month, index_value, amount, is_paid FROM expenses WHERE user_id = ?", (user_id,))
        expenses = cursor.fetchall()
    
    user_currency = g.user['currency']
    df = pd.DataFrame(expenses, columns=["Service", "Month", "Index", "Amount", "Paid"])
